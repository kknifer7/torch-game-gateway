...
public class DeaconController {
    ...
    @Autowired
    public DeaconController(Fuse fuse, CallRecordService callRecordService, GlobalGatewayContainer gatewayContainer, Loadbalancer loadbalancer, GatewayAgent gatewayAgent) {
        this.fuse = fuse;
    ...
    public Mono<?> all(@PathVariable("serviceId") String serviceId, ServerWebExchange exchange){
        Gateway gateway = (Gateway) loadbalancer.choose(gatewayContainer.getAll(), requestContext);
